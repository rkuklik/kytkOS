...
in {
  options.kytkos.net.blacklist = {
    enable = mkEnableOption "Base blocklist (malware + adware)" // {default = true;};
  ...
  config = {
    warnings = with cfg;
      if ((fakenews || gambling || porn || social) -> enable)
      then []
      else ["Additional blacklists supported if base blocklist is enabled"];
